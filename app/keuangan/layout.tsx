...
import { KeuanganSidebar } from "@/components/admin/sidebar-keuangan";
import { Menu, Bell, LogOut, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useRouter } from "next/navigation";

const ALLOWED = ["SUPER_ADMIN", "ADMIN_KEUANGAN"];
...
export default function KeuanganLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!checked) {
    return <div className="min-h-screen bg-[#FCFCFD] flex items-center justify-center text-sm text-slate-500">Memuat keuangan...</div>;
  }
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className="fixed left-0 top-0 h-[100dvh] w-[85vw] max-w-[300px] bg-white shadow-2xl flex flex-col">
            ...
            <div className="flex-1 overflow-auto">
              <KeuanganSidebar onNavigate={() => setOpen(false)} />
            </div>
          </aside>
        </div>
      )}
      ...
      <div className="lg:pl-[256px] min-w-0">
        <header className="sticky top-0 z-20 flex h-[56px] items-center justify-between border-b border-slate-100 bg-white/90 backdrop-blur-md px-4 sm:px-6 lg:px-8 gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="text-sm font-medium tracking-tight text-slate-900 truncate">Keuangan</div>
