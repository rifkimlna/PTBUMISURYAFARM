...
// Pemesanan (PO) bisa dibuat langsung atau dari Penawaran (?dari=<id>).
export default async function PemesananBaruPage({
  searchParams,
}: {
  ...
  if (dari) {
    ...
    if (!sumber || sumber.tipe !== "PENAWARAN") notFound();
    asalLabel = `Dari ${sumber.noDokumen} (${sumber.supplier?.nama ?? "-"})`;
    prefill = {
      id: "",
      header: {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">Pemesanan Pembelian</h1>
        <p className="mt-1 text-sm text-slate-400">
          {asalLabel || "Purchase order ke supplier"}
          {prefill && " · Data terbawa otomatis dari dokumen asal"}
        </p>
      </div>
      <DokumenBeliForm tipe="PESANAN" initial={prefill} />
    </div>
  );
}
